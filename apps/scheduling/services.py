...
from .models import Assignment, Shift, ShiftStatus
...
def validate_employee_no_overlap(employee_id: int, shift: Shift) -> None:
    overlapping = (
        Shift.objects.filter(assignments__employee_id=employee_id, date=shift.date, is_deleted=False)
        .exclude(id=shift.id)
...
def publish_drafts_in_range(*, manager_id: int, start: date, end: date) -> int:
...
def delete_drafts_in_range(*, manager_id: int, start: date, end: date) -> int:
    qs = Shift.objects.filter(
        created_by_id=manager_id,
        date__gte=start,
        date__lte=end,
...
def shifts_for_manager(
    *,
    ...
    qs = (
        Shift.objects.filter(created_by_id=manager_id, date__gte=start, date__lte=end, is_deleted=False)
